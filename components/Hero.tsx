...
import { BookOpenIcon, EyeIcon, WrenchIcon, VideoCameraIcon } from '@heroicons/react/24/outline';
import { FireIcon, BugAntIcon, SignalIcon } from '@heroicons/react/24/solid';
      ...
      <div className={`relative w-20 h-28 md:w-28 md:h-40 rounded-lg backdrop-blur-md transition-all duration-1000 ${stage === 2 ? 'bg-zinc-800/40 border-zinc-500/50 shadow-xl scale-110 -translate-y-4' : 'bg-zinc-900/10 border-zinc-800 scale-100 border border-dashed'}`}>
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <div className={`absolute transition-all duration-700 flex flex-col items-center ${stage === 2 ? 'opacity-100 scale-100 blur-0' : 'opacity-0 scale-75 blur-sm'}`}>
             <FinalIcon className="w-10 h-10 md:w-14 md:h-14 text-purple-500" />
             {stage === 2 && (
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        ...
        {/* Bottom Right: Item -> Tool */}
        <div className="hidden md:block">
            <DrawingTransformation 
            initialIcon={WrenchIcon} 
            finalIcon={SignalIcon} 
            label="EMF"
            delay={3000} 
            x="88%" 
            y="75%"
        ...
        {/* Top Right: Bug -> Mechanic */}
        <div className="hidden lg:block">
            <DrawingTransformation 
            initialIcon={BugAntIcon} 
            finalIcon={EyeIcon} 
            label="SANITY"
            delay={6000} 
            x="88%" 
        ...
        {/* Bottom Left: Hunt -> Logic */}
        <div className="hidden md:block">
            <DrawingTransformation 
            initialIcon={FireIcon} 
            finalIcon={WrenchIcon} 
            label="HUNT"
            delay={4500} 
            x="5%" 
            y="72%"
